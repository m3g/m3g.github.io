...
A linguagem de escolha do curso é Julia, nas suas versões mais
recentes (isto é, a partir da versão 1.0). A escolha desta linguagem se
deve à simplicidade de sua sintaxe e à eficiência numérica dos programas
gerados. Julia é uma linguagem idealizada, desde o princípio, para a
computação numérica, mas permite a interação fácil com linguagens muito
populares, como Python, Fortran e C, que tem muitas bibliotecas prontas. 

O site oficial da linguagem Julia é
\begin{center}
http://julialang.org
\end{center}
A documentação apresentada no site é muito detalhada e completa, e pode
ser usada como referência permanente. Entre nesse site, e instale o
Julia no seu computador. 

Há vários cursos, tutoriais e livros sobre Julia. Uma busca na
internet por ``Introdução a Julia'', em qualquer idioma, permite o
acesso a uma grande variedade de material gratuito. Este curso, no
entanto, não é um curso de Julia. Este é um curso de fundamentos de
simulação. As simulações requerem uma linguagem de programação, e o
Julia é a linguagem de escolha, porque sua sintaxe é simples e
...
seus próprios problemas aplicados. Quando aprendemos a programar em uma
linguagem, passar para qualquer outra é sempre bastante fácil.
...
\end{code}
Todas as linhas que começam com uma {\it cerquilha} (o hashtag), 
"\#", são ignoradas, sendo
chamadas de ``comentários''. Os comentários ajudam, em programas
complexos, a entender o que está sendo feito. Nossos programas vão
começar sempre com esse {\tt let Program} e terminar com esse {\tt end}.
Isto não é obrigatório, mas muito mais adiante vamos explicar esta
escolha. Não se preocupe com isto agora, entenda que isto apenas define
que isso define onde o programa começa e termina. 

Este código pode ser editado em qualquer editor de texto básico (como o
Notepad, Vim, etc. Uma forma de trabalhar, no entanto, é usando um
editor especializado, como o Juno:
\begin{center}
{\tt http://junolab.org/}
\end{center}
Instale este editor em seu computador também, se quiser.

Vai ser fundamental escrever o resultado dos programas "na tela". Em
Julia o comando mais útil para isto é o {\tt println} (de "imprimir em
uma linha"):
\begin{code}
let Program
  # This is a commentary
...
Uma das estruturas mais fundamentais em programação são os {\it loops}.
Há diferentes maneiras de escrever um loop em Julia, sendo a mair comum 
a que usa o comando "para ... em ... ",
\begin{code}
a = 0
for i in 1:3
  a = a + 1
...
\codefile{./codes/randomsearch.jl}
