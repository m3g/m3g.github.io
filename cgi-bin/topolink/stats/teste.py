...
for linktype in linker_data :
  ...
  print(residue1,residue2,dist)

  try : 
    data_file='./teste_data/'+protein_type+'_'+residue1+residue2+'.dist'
    ...
    data_file='./teste_data/'+protein_type+'_'+residue2+residue1+'.dist'
    avg_size, linker_length, link_count_avg, pdb_count, size_min, size_max = \
       loadtxt(data_file,usecols=(0,1,2,4,5,6),unpack=True,comments="#",dtype=float)
  ...
  for value in linker_length :
    #print(value,linker_length[i],size_min[i],size_max[i])
    if i == ndata : 
      break

    #print(dist, linker_length[i], linker_length[i+1])
