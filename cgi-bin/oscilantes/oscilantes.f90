...
program oscilantes
  ...
  call getarg(12,file)                            ! Nome do arquivo de saida
  ...
  write(10,"( t1,'#',t10,'Tempo',t37,'CA',t63,'CB',t89,'CC',t115,'CD' )")
  ...
  do i = 1, nsteps-1
    ...
    if ( error > 1.d-3 ) then
      write(10,"( '# ERRO: Simulação instável. Erro na soma das concentrações = ', e12.7 )") error
      stop
    end if
    if ( CA > mass_balance .or. CA < 0 .or. &
      ...
      write(10,"( '# ERRO: Simulação instável. Concentrações ficaram sem sentido.' )")
      stop
    end if

    ! Se o resto da divisão de i por iprint for 0, escrever as concentrações
      
    if ( mod(i,iprint) == 0 ) write(10,*) (i-1)*dt, CA, CB, CC, CD
